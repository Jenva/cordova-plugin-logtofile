...
public class LogToFile extends CordovaPlugin {
    ...
    private void configureLogger(int size, int limit) {
        ...
        if (limit == 0) {
            ...
            rollingPolicy.setMaxIndex(limit);
        }
        ...
        if (size == 0) {
            ...
            triggeringPolicy.setMaxFileSize(size + "MB");
        }
    ...
    public boolean execute(String action, JSONArray data, final CallbackContext callbackContext) throws JSONException {
        ...
        if (action.equals("debug")) {
            ...
            final int size = data.getInt(1);
            final int limit = data.getInt(2);
            cordova.getThreadPool().execute(new Runnable() {
                public void run() {
                    try {
